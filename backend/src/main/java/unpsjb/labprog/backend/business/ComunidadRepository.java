...
LocalDateTime obtenerFechaIngreso(Long idMiembro, Long idComunidad);
        ...
        @Query("MATCH (u:Usuario) WHERE id(u) = $idUsuario " +
                        "CREATE (c:Comunidad {nombre: $nombre, fechaDeCreacion: $fechaCreacion, latitud: $latitud, longitud: $longitud, genero: $genero, descripcion: $descripcion, cantidadMaximaMiembros: $participantes, esPrivada: $privada, esModerada: $moderada, imagen: $imagen, ubicacion: $ubicacion})"
                        +
                        " CREATE (u)<-[:CREADA_POR {fechaCreacion: $fechaCreacion}]-(c) " +
                        "RETURN c")
        Comunidad guardarComunidadYCreador(String nombre, String genero, String descripcion, int participantes, boolean privada, boolean moderada,
       ...
       void desetiquetarComunidad(Long comunidadId, Long etiquetaId);
        ...
        @Query("MATCH (c:Comunidad)-[:ETIQUETADA_CON]->(etiqueta:Etiqueta) " +
                        "WHERE NOT EXISTS { " +
                        "  MATCH (u:Usuario)-[:MIEMBRO|ADMINISTRADA_POR|CREADA_POR|MODERADA_POR]-(c) " +
                        "  WHERE id(u) = $idUsuario " +
                        "} " +
                        "AND NOT (u)-[:EXPULSADO_COMUNIDAD]-(comunidad) " +
        ...
        @Query("""
                            ...
                            WITH c, count(DISTINCT u) AS totalUsuarios, us
                            WHERE totalUsuarios >= $min AND totalUsuarios <= $max
                            AND (c)-[:MIEMBRO|ADMINISTRADA_POR|CREADA_POR|MODERADA_POR]-(us) // Verifica si el usuario es miembro de la comunidad
                            RETURN DISTINCT c
                        """)
